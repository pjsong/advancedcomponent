...
public class RegController extends BaseController {
	public void process(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if(act.equals("")){
			...
			VelocityParserFactory.getVP().render("register_eng", vc, request, response);
			return;
		}
		if(act.equals("regyes")){
			VelocityContext vc=new VelocityContext();
			new GlobalVariablesBA().setCommonVariables(request, vc);
	        VelocityParserFactory.getVP().render("registerYes_eng", vc, request, response);
	        return;
		}
	...
	protected void registerUser(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if(error.size()>0){
			...
			VelocityParserFactory.getVP().render("registerDone_eng", vc, request, response);
		}
	}
